CREATE OR REPLACE VIEW selectTicketFromFermat AS SELECT * FROM glpi_tickets WHERE location = 'Fermat';
CREATE OR REPLACE VIEW selectTicketFromCondorcet AS SELECT * FROM glpi_tickets WHERE location = 'Condorcet';
CREATE OR REPLACE VIEW selectTicketFromTuring AS SELECT * FROM glpi_tickets WHERE location = 'Turing';
CREATE OR REPLACE VIEW selectTicketFromCauchy AS SELECT * FROM glpi_tickets WHERE location = 'Cauchy';
CREATE OR REPLACE VIEW selectTicketFromMartin AS SELECT * FROM glpi_tickets WHERE location = 'Saint Martin';
CREATE OR REPLACE VIEW selectTicketFromGermain AS SELECT * FROM glpi_tickets WHERE location = 'Saint Germain';

...
CREATE OR REPLACE PROCEDURE close_ticket_pau (
...
BEGIN
    ...
    FROM glpiAdmin_pau.glpi_tickets
    WHERE id = p_ticket_id;

    -- Insérer dans la table glpi_treated_tickets en fonction des données du ticket existant
    EXECUTE IMMEDIATE '
        INSERT INTO glpiAdmin_pau.glpi_treated_tickets (id, ticket_id, entites_id, name, creation_date, closedDate, solvedStatus, previousPriority, location)
        SELECT glpiAdmin_pau.glpi_treated_tickets_seq.NEXTVAL, id, entites_id, name, creation_date, SYSTIMESTAMP, CASE WHEN :tech_role_exists > 0 THEN 1 ELSE 0 END, priority, location
        FROM glpiAdmin_pau.glpi_tickets
        WHERE id = :ticket_id'
    USING v_technician_role_exists, p_ticket_id;

    -- Supprimer le ticket traité de la table glpi_tickets
    DELETE FROM glpiAdmin_pau.glpi_notifications WHERE tickets_id = p_ticket_id;
    DELETE FROM glpiAdmin_pau.glpi_tickets WHERE id = p_ticket_id;
...
GRANT EXECUTE ON close_ticket_pau to technician_role;
...
BEGIN
    ...
    FROM glpiAdmin_pau.glpi_treated_tickets
    WHERE ticket_id = p_ticket_id;

    -- Insérer dans la table glpi_tickets en fonction des données du ticket traité
    EXECUTE IMMEDIATE '
        INSERT INTO glpiAdmin_pau.glpi_tickets (id, entites_id, name, creation_date, user_id_last_updater, status, priority, location)
    ...
    DELETE FROM glpiAdmin_pau.glpi_treated_tickets WHERE ticket_id = p_ticket_id;
...
GRANT EXECUTE ON reopen_ticket_pau to technician_role;
GRANT EXECUTE ON reopen_ticket_pau to simple_user_role;